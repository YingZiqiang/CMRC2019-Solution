...
class BertMultiAttention(nn.Module):
    ...
    def forward(self, enc_hidden_states,dec_hidden_states, attention_mask):
...
class BertDecoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_deencoded_layers=True):
        all_decoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask)
            if output_all_deencoded_layers:
                all_deencoder_layers.append(hidden_states)
        if not output_all_deencoded_layers:
            all_deencoder_layers.append(hidden_states)
        return all_deencoder_layers
...
class BertForQuestionAnswering(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
    #def forward(self, enc_hidden_states,dec_hidden_states, attention_mask):
        sequence_output_multi = self.bert.BertMultiAttention.forward(sequence_output,sequence_output,attention_mask)
        logits = self.qa_outputs(sequence_output_multi)
...
class BertForCMRC2019Dec(PreTrainedBertModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertForCMRC2019Dec, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)

        # self.atten = BertMultiAttention(config)
