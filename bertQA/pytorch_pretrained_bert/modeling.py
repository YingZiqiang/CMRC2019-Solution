...
class BertForCMRC2019(PreTrainedBertModel):
    """BERT model for Question Answering (cmrc2019).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    Inputs:
        ...
        `positions`: position of the token for the label position: torch.LongTensor of shape [batch_size].
            Positions are clamped to the length of the sequence and position outside of the sequence are not taken
            into account for computing the loss.

    Outputs:
        if `positions` is not `None`:
            Outputs the total_loss which is the CrossEntropy
        if `positions` is `None`:
            Outputs the logits respectively for the tokens of shape [batch_size, sequence_length].
    ...
    def __init__(self, config):
        super(BertForCMRC2019, self).__init__(config)
        self.bert = BertModel(config)
        self.decoder = BertDirectedAttention(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, answer_mask=None, positions=None):
        ...
        #Start and End Self Attention for span predicition
        encoded_skip=self.skip_encoder(sequence_output1d,extended_attention_mask,output_all_encoded_layers=False)
        # encoded_skip_end=self.skip_encoder_end(sequence_output1d,extended_attention_mask,output_all_encoded_layers=False)
        # for_end = torch.cat((encoded_skip_end[-1],encoded_skip_start[-1]),-1)
        ...
        # seq_end,_  = self.endLSTM(for_end)
        logits = self.qa_outputs(encoded_skip[-1])
        # end_logits = self.qa_outputs_end(seq_end)
